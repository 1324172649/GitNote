...
报表列表的单个报表有“导出数据”按钮，可以选择导出哪个时间段的数据（最长可选跨度为 6 个月）。导出格式为 PDF（由开发提前开发未指定的报表样式）。
...
报表列表的单个报表有“导出数据”按钮，可通过组织架构选择导出哪些门店的数据，也可以选择导出哪个时间段的数据（最长可选跨度为 6 个月）。导出格式为 PDF（由开发提前开发未指定的报表样式）。
...
|10|    |表格|2列×2行|/|    |
|11|    |地理位置|当前最近的一个位置|/|    |
|12|    |表单|/|/|可选择当前在使用的套装中有被使用的表单，指定某一次提交的数据插入。只是提交过的常用表单；可选择下架登记表和快速评估表。|
